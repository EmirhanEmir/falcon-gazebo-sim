...
## 23. Engine-Out and Asymmetric Thrust — Classification Record (`ENGINE_OUT_AND_ASYMMETRIC_THRUST_VALIDATION`, 2026-08-27)

**Scope: interpretation/classification only.** No coefficient, lookup table, derivative, or the `high_alpha_limiter`/beta-saturation status (§21) was retuned or touched to produce this section. Data source: `tests/gazebo/results/engine_out_asymmetric_thrust_result.json` (Part 0 analytic sign pre-registration, Part 1 static differential-thrust matrix, Part 2 engine-out transients, Part 3/4 rudder-compensation search, Part 5 surviving-engine throttle adequacy, Part 6 single-engine trim, Part 7 single-engine free 6-DOF flight, Part 8 speed-dependence rudder-requirement-only sweep at 16/24 m/s, Part 9 rudder yaw-authority margin, Part 10 safety summary) and `tests/gazebo/results/engine_out_asymmetric_thrust_log.txt`, both produced by `gazebo-testing`. Cross-referenced against `docs/source_of_truth/propulsion/propulsion_v1_config.yaml` (`rpm_cap_v1=11500.0`, `current_limit_A=65.0`) and `docs/source_of_truth/aerodynamics/aero_v1_config.yaml` (rudder/aileron wide-deflection lookup tables, `Clda_per_rad`/`Cl` reference values), and `docs/source_of_truth/controls/actuator_v1_config.yaml` (±45° mechanical actuator range). This record persists the classification `aerodynamics` reported back to the coordinator for this stage, so it is reviewable in-repo rather than only existing in a chat transcript (per the explicit process-gap correction from §22/`FLIGHT_ENVELOPE_VALIDATION`).

All Part 0 sign pre-registrations (Mz yaw-toward-dead-engine, rudder-toward-good-engine, reaction-torque roll direction) confirmed 3/3, and every mirror (left-engine-out vs. right-engine-out) pair matched to <1e-6 N·m — the classification below rests on a self-consistent, symmetric dataset with no NaN/Inf, no crash, and no unexplained asymmetry anywhere in the suite.

### 23.1 Three-tier engine-out region classification

| V (m/s) | Tier | Reasoning |
|---|---|---|
| 18.166 (nominal) | `ENGINE_OUT_VALIDATED_REGION` | Full evidence chain present and self-consistent: rudder-compensation search converges cleanly to ≈17.0–17.1° both sides (residual Mz 0.0006–0.0013 N·m), surviving-engine throttle 0.5614 confirmed sufficient with comfortable headroom (T/D≈0.9995, RPM 58% of the 11500 cap, current 29% of the 65 A limit, `any_rpm_cap=False`/`any_current_limited=False`), single-engine trim converges (Lift/W=0.992, T/D≈0.96–0.962, Mz≈0.001 N·m), and a full 13 s single-engine free-6-DOF-flight run exists on **both** sides, classified `PASS_WITH_DRIFT` — no NaN, modest altitude/airspeed drift (-1.82/-1.83 m, +0.21/+0.21 m/s), and the roll/yaw drift fully traced to the known, disclosed, uncorrected Mx residual (see §23.2). This is the only speed with a completed engine-out free-flight test, matching §22's own precedent for what earns the top (`VALIDATED_CORE_ENVELOPE`/here `ENGINE_OUT_VALIDATED_REGION`) tier. |
| 16.0 | `ENGINE_OUT_PROVISIONAL_REGION` | Rudder-authority physics is clean and shows margin: required rudder ≈21.4–21.5° (larger than nominal, correctly tracking the expected 1/qbar scaling as V drops), residual Mz small (0.0003–0.0017 N·m), and neither `LOW_RUDDER_MARGIN` (>25°) nor `RUDDER_SATURATION_RISK` (near ±45°) triggered — roughly 23.5° of additional rudder throw remains before the mechanical limit. However, Part 8 at this stage is deliberately "minimal": **no single-engine trim or free-flight run was performed at 16 m/s** — only the rudder-requirement calculation, itself built on a two-engine baseline (elevator/throttle/drag) reused read-only from `flight_envelope_result.json` rather than re-searched. That same two-engine baseline was itself only `PROVISIONAL_EDGE_ENVELOPE` in §22.1 (no two-engine free-flight run at 16 m/s either) — an engine-out condition layered on an already-provisional baseline cannot be promoted past provisional on rudder-authority math alone, however clean that math is. |
| 24.0 | `ENGINE_OUT_PROVISIONAL_REGION` | Rudder-authority physics is equally clean: required rudder ≈13.0–13.1° (smaller than nominal, again correctly tracking qbar scaling as V rises), residual Mz small (0.0002–0.0041 N·m), well clear of both margin flags, with roughly 32° of reserve rudder throw remaining. The two-engine baseline at 24 m/s is stronger than at 16 m/s — it was `VALIDATED_CORE_ENVELOPE` in §22.1, corroborated by an actual two-engine free-flight `PASS`. But Part 8 at 24 m/s is, identically to 16 m/s, rudder-requirement-only for the **engine-out** condition specifically — no single-engine trim or free-flight run exists at this speed. A strong two-engine baseline does not substitute for a missing engine-out-specific trim/free-flight run, so this speed is provisional for the same evidentiary reason as 16 m/s, not for any physics red flag. |

No speed reached `ENGINE_OUT_NOT_MAINTAINABLE` — no case anywhere in the suite showed `LOW_RUDDER_MARGIN`, `RUDDER_SATURATION_RISK`, an infeasible surviving-engine throttle, a propulsion/actuator limit hit, or a NaN/divergent free-flight result.

### 23.2 Interpretation of the `PASS_WITH_DRIFT` roll/yaw residual (nominal speed)

The nominal single-engine free-flight run's roll drift (reaching ≈18.38°/-18.39° over ~13 s, both sides) and associated yaw excursion (max\|yaw-start\|≈65°) trace directly to the Mx≈+0.2342/-0.2340 N·m residual already disclosed at the Part 6 trim stage — this is the *same* reaction-torque-dominated roll moment left uncorrected because the task instruction was to attempt rudder-only compensation first and add aileron only if genuinely needed; it is not a new or surprise effect discovered in free flight, and it is symmetric to <0.001 N·m between the two engine-out directions.

Assessed against available aileron roll authority: using the small-signal reference `Clda_per_rad=0.414/rad` (`aero_v1_config.yaml`, `lateral_directional.Clda_per_rad`) and the trim-condition dynamic pressure (`qbar≈202.1 Pa`, `S=0.4514 m²`, `b=2.093 m`), the roll coefficient needed to null Mx=0.234 N·m is `Cl_needed = Mx/(qbar·S·b) ≈ 0.234/190.9 ≈ 0.00123`. Dividing by `Clda_per_rad` gives a required deflection of only **≈0.17°** — cross-checked directly against the actual wide-deflection aileron lookup table (`control_surface_lookup.aileron.Cl`, which is what `AeroModel.hh` actually evaluates, not the linear scalar): linearly interpolating between the `delta_a=0` (`Cl=0`) and `delta_a=+2°` (`Cl=0.01447`) breakpoints for `Cl_needed=0.00123` gives the same ≈0.17°, confirming the small-signal estimate. This deflection is roughly an order of magnitude below even the smallest tested breakpoint (±2°), sits deep inside the `HIGH_CONFIDENCE_SMALL_SIGNAL` band (≤10°), and is a trivial fraction of the ±45° mechanical actuator range (`actuator_v1_config.yaml`) — consistent with `gazebo-testing`'s own Part 6 note that "~5° aileron authority is on the order of several N·m at this qbar," i.e. two orders of magnitude more authority than the ≈0.234 N·m residual requires.

**Verdict:** this is an acceptable, expected, and well-explained V1 result, not a real concern, and it does **not** push the nominal-speed classification down from `ENGINE_OUT_VALIDATED_REGION`. The magnitude, sign, and symmetry of the residual are fully accounted for by a known, disclosed test-design choice (aileron deliberately left neutral), the required corrective deflection is trivially small relative to both the linear-derivative validity range and the mechanical range, and the free-flight run itself stayed bounded and NaN-free over the full 13 s with only modest altitude/airspeed drift — i.e. the aircraft is not diverging, it is executing exactly the slow, roll-damping-limited drift physically expected from an un-trimmed, otherwise-small constant disturbance torque (`Clp=-0.54187` provides the roll-rate damping that keeps this bounded rather than runaway). No aileron trim was added as part of this task — this assessment is feasibility-only, per the task's explicit instruction not to add it.

### 23.3 Rudder yaw-authority margin (nominal speed)

At the nominal speed, the propulsive out-yaw moment from a single engine loss is `Mz_prop≈±1.5432 N·m`, and full compensation is achieved at ≈17.0–17.1° of rudder (residual ≤0.0013 N·m). Measured at representative practical deflections (`part9` margins, defined as `|aero_Mz(delta)|/|Mz_prop|`): 10° of rudder supplies only ≈58% of what full compensation needs (insufficient alone), 15° supplies ≈88% (close but still short), and 25° supplies ≈147% (comfortably exceeds requirement, i.e. rudder alone can more than counter the engine-out yawing moment well before reaching its mechanical limit) — leaving roughly 28° of additional rudder throw as reserve beyond the ≈17° trim point, out of the ±45° mechanical range, with neither `LOW_RUDDER_MARGIN` nor `RUDDER_SATURATION_RISK` triggered at any tested speed. This `RUDDER_YAW_AUTHORITY_MARGIN` figure is derived entirely from this project's V1 XFLR5/VLM-based wide-deflection rudder lookup and the V1 propulsion reaction-torque model — it is a **Gazebo-model-only figure**, not a certification-grade or flight-test-validated margin, and carries the same `LOW_CONFIDENCE_HIGH_DEFLECTION_REFERENCE` caveat as any other use of this lookup table beyond ±25°.

### 23.4 Addendum, 2026-08-27 — nominal-speed evidentiary basis upgraded to the 29.1 s extended free-flight (`ENGINE_OUT_AND_ASYMMETRIC_THRUST_VALIDATION` validation follow-up)

**This addendum is additive; §23.1 and §23.2 above are left as the historical record of the original (~13 s) pass and are not rewritten.** `validation` correctly flagged a MAJOR gap in that original evidence: both single-engine free-flight runs ended their ~13 s window with roll/yaw still at the window maximum, with no persisted evidence of leveling off — "bounded through 13 s" was not the same claim as "bounded." `gazebo-testing`'s follow-up extended the identical single-engine free-flight (same throttle/rudder/aileron-neutral conditions) to a 29.1 s window on both sides and added a plateau-vs-continued-growth trend check on the never-wrapping body rates (`p_deg_s`/`r_deg_s`), reported in the new `part7_free_flight_extended_30s` JSON field.

**Corrected/extended result (both sides, mirror-symmetric):** roll genuinely settles — `LEFT_OUT` ends at roll=+21.43° (max 22.82°), `RIGHT_OUT` ends at roll=-21.44° (max -22.83°), with `roll_trend=NEGLIGIBLE_RATE_SETTLED` (roll rate decaying from ≈±0.198°/s in the middle third of the window to ≈±0.148°/s in the last third — i.e. `roll_at_window_end_is_window_max=False`, the opposite of the original 13 s snapshot). Yaw rate levels off rather than continuing to accelerate — `yaw_trend=RATE_LEVELED_OFF_STEADY_NONZERO`, settling to a steady ≈∓10.0°/s (last-third mean) vs. ≈∓9.1°/s (mid-third mean), ratio≈1.10 — a steady turn rate, not unbounded growth. Altitude/airspeed drift over the full 29.1 s remain modest (alt≈-6.46/-6.46 m, V-drift≈+0.69/+0.69 m/s) and `any_nan=False` throughout.

**Effect on this document:** this confirms — it does **not** contradict — §23.1's `ENGINE_OUT_VALIDATED_REGION` call and §23.2's "not diverging" verdict for 18.166 m/s. The correct physical picture is a legitimate bounded engine-out flight condition: a **steady, mildly-banked (~21–22°) turn under rudder-only compensation**, roll-rate-settled and yaw-rate-leveled, not a spiral divergence — consistent with §23.2's `Clp`-damping reasoning, but now demonstrated by an actually-settled trend rather than inferred from an unsettled 13 s snapshot. Going forward, §23.1's nominal-speed row and §23.2's "bounded... not diverging" language should be read together with this addendum's steady-bank/decaying-roll-rate/leveled-yaw-rate evidence, not with the original 13 s "no divergence within the window" observation alone, which is the weaker claim this addendum supersedes. No coefficient, lookup table, derivative, or aircraft physics parameter was changed by this addendum or by the underlying re-run — only the free-flight duration and the trend-analysis method (never-wrapping body rates instead of the wrapping Euler yaw angle used implicitly before) were corrected. Data source: `tests/gazebo/results/engine_out_asymmetric_thrust_result.json`'s `part7_free_flight_extended_30s` field, `tests/gazebo/results/engine_out_asymmetric_thrust_log.txt`, and `tests/gazebo/README.md`'s 2026-08-27 follow-up entry.
